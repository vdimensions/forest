...
//    abstract Path: Forest.Path with get
...
//    abstract member Remove: path: Forest.Path -> IDomIndex
//    abstract member Insert: path: Forest.Path -> node: IDomNode -> IDomIndex
//    abstract member Clear: unit -> IDomIndex
//    abstract member TryFind : path: Forest.Path -> Option<IIndex<IDomNode, string>>
//    abstract member Count: int with get
//    abstract member Paths: IEnumerable<Forest.Path> with get
//    abstract member Item: Forest.Path -> IIndex<IDomNode, string> with get
//
//type [<AbstractClass>] AbstractDomIndex<'T when 'T:> AbstractDomIndex<'T>>() as self =
//    abstract member Add: node: IDomNode -> 'T
//    abstract member Insert: path: Forest.Path -> node: IDomNode -> 'T
//    abstract member Remove: path: Forest.Path -> 'T
//    abstract member Remove: node: IDomNode -> 'T
//    abstract member Clear: unit -> 'T
//    abstract member TryFind : path: Forest.Path -> Option<IIndex<IDomNode, string>>
//    abstract member ContainsPath: path: Forest.Path -> bool 
//    abstract member Count: int with get
//    abstract member Paths: IEnumerable<Forest.Path>
//    abstract member Item: Forest.Path -> IIndex<IDomNode, string> with get
...
//        member this.Remove (path:Forest.Path) = upcast self.Remove path : IDomIndex
//        member this.Remove (node:IDomNode) = upcast self.Remove node : IDomIndex
//        member this.Clear () = upcast self.Clear () : IDomIndex
//        member this.TryFind path = self.TryFind path
//        member this.Count = self.Count
//        member this.Paths = self.Paths
//        member this.Item with get path = self.[path]
//
//type [<Sealed>] DefaultDomIndex(index: IWriteableIndex<IAutoIndex<IDomNode, string>, Forest.Path>) as self =
//    inherit AbstractDomIndex<DefaultDomIndex>()
//    let comparer = StringComparer.Ordinal
//    new() = new DefaultDomIndex(new WriteableIndex<IAutoIndex<IDomNode, string>, Forest.Path>())
...
//       match index.TryFind path with | Some ni -> nodeIndex <- ni | None -> ()
//       nodeIndex <- nodeIndex.Add node
//       new DefaultDomIndex((index.Remove(path).Insert path nodeIndex))
//
//    override this.Remove (path: Forest.Path) = new DefaultDomIndex((index.Remove path))
//    override this.Remove (node: IDomNode) = 
//        let path = node.Path
//        let parentPath = path.Parent
//        let nodeIndex = index.TryFind parentPath;
...
//    override this.TryFind path = match index.TryFind path with | Some r -> Some (upcast r) | None -> None
//    override this.ContainsPath path = index.ContainsKey path
//    override this.Count = index.Count
//    override this.Paths = index.Keys
//    override this.Item with get k = index.[k] |> (fun x -> upcast x)
//
//type [<Sealed>] internal ViewNode(path: Forest.Path, descriptor: IViewDescriptor) as self =
...
//
//type [<Sealed>] internal RegionNode(path: Forest.Path, name: string) as self =
