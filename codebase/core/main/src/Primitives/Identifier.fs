...
type Identifier = 
    | Shell_
    | ViewID_ of Identifier*string*string*ForestID
    [<CompiledName("Shell")>]
    ...
    member this.UniqueID with get() = match this with Shell_ -> ForestID.empty | ViewID_ (_, _, _, fid) -> fid
...
module internal Identifier =
    ...
    let addNew region name parent = add (ForestID.newID()) region name parent
