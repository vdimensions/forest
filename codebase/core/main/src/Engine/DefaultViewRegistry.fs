...
type [<AbstractClass;NoComparison>] AbstractViewRegistry(reg : IViewRegistry, factory : IViewFactory) = 
    ...
    //member this.RegisterT when 'T :> IView> () = 
...
type [<Sealed;NoComparison>] internal DefaultViewRegistry (factory : IViewFactory, reflectionProvider : IReflectionProvider) = 
    inherit AbstractViewRegistry(Forest.ComponentModel.ViewRegistry(), factory)
    ...
    //            Ok (upcast ViewDescriptor(vn, viewType, viewModelType, commandsIndex, linksIndex, eventSubscriptions, viewType.ExtendsOrImplements<ISystemView>()) : IViewDescriptor)
    //        | (ce, ee) ->
