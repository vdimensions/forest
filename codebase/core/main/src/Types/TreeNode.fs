...
type [<CustomComparison;CustomEquality>] TreeNode = 
    ...
    member this.Region with get() = match this with Shell_ -> rname.Empty | ViewID_ (_, r, _, _) -> r
    member this.View with get() = match this with Shell_ -> vname.Empty | ViewID_ (_, _, v, _) -> v
